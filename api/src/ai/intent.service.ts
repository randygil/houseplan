...
import { buildIntentPrompt, INTENT_SCHEMA, Item, normalizeIntent, Parsed, txLine } from './intent';
...
export class IntentService {
  ...
  /** One LLM call: intent + extraction, with the short context of PLAN 3.2. */
  async parse(text: string): Promise<Parsed> {
    const now = new Date();
    const [accounts, categories, turns, recent, pending] = await Promise.all([
      this.ledger.balances(),
      this.cats.list(),
      this.db.chatTurn.findMany({ orderBy: { id: 'desc' }, take: 6 }), // includes the current user turn
      this.ledger.recent(5),
    ...
    return normalizeIntent(await this.llm.json(prompt, INTENT_SCHEMA), { now, accounts });
  }
